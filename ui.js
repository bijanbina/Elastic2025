...
function rcube_elastic_ui()
{
  ...
  //  this.compose_status = compose_status;
	...
	this.screen_resize_headers = screen_resize_headers;
                ...
                rcmail.addEventListener('listupdate', function(prop) {
					...
					*/
					if(rcmail.task== "mail")
						...
						rcmail.enable_command('e2022-filtermenu', true);
...
/*
			...
			// plugin persistent login
			 setTimeout(function() {
				...
				$('#rcmloginpwd').parent().append(html);
				})
		}
...
*/
     ...
     *  screen mode
     */
    function screen_mode() // elastic2022, only : small, normal, large
    {
...
window.removeEventListener("touchend",l),window.removeEventListener("touchmove",o,d)),window.removeEventListener("scroll",a)}}},i={distThreshold:60,distMax:80,distReload:80,distIgnore:0,mainElement:"body",triggerElement:"body",ptrElement:".ptr",classPrefix:"ptr--",cssProp:"min-height",iconArrow:"&#8675;",iconRefreshing:"&hellip;",instructionsPullToRefresh:"Pull down to refresh",instructionsReleaseToRefresh:"Release to refresh",instructionsRefreshing:"Refreshing",refreshTimeout:500,
...
//delete "normal" priority text
$('.header-headers .priority .prio3').closest('tr').remove()
...
if(elm)
	...
	setTimeout(function() {audioanim()}, 50);
	}
}
...
function e2022_sortmenu_start()
{
$('#listsort-menu .submit').removeClass('submit')

var sort_col = rcmail.env.sort_col
if(!sort_col)sort_col="arrival"

$('#listsort-menu .'+sort_col).addClass('submit')
$('#listsort-menu .'+rcmail.env.sort_order).addClass('submit')
}

function e2022_sortmenu()
{
var t=event.target
t.closest('ul').querySelector('.submit').classList.remove("submit")

t.classList.add("submit")

e2022_sortmenu_refresh()
}

function e2022_sortmenu_refresh()
{
var sort_order = [... document.querySelector('.sort_order .submit').classList].filter((w) => w!="submit")[0]
var sort_col = [... document.querySelector('.sort_col .submit').classList].filter((w) => w!="submit")[0]
rcmail.set_list_sorting(sort_col, sort_order);
rcmail.list_mailbox('', '', sort_col+'_'+sort_order);
}

function e2022_filtermenu()
{
var t=event.target
var sel = [... t.classList].filter((w) => w.startsWith('_'))[0].replace("_", '')

if(sel=="WITHATTACHMENT")sel = "OR OR OR HEADER Content-Type application/ HEADER Content-Type multipart/mixed HEADER Content-Type multipart/signed HEADER Content-Type multipart/report"
